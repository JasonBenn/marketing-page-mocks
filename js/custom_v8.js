	...
	jQuery('#fullpage').fullpage({
		...
		afterLoad: function(anchorLink, index){
			...
			if(index == 2){
				...
				jQuery('.shine').delay(500).fadeTo(0,0.65).animate({'background-position-y': '-300%'}, 800,function(){
			...
			if(index == 3){
				...
				function swipeCard(){
					...
					//jQuery('.card').finish().css({opacity:1}).rotate({duration:0, animateTo:0,center: ["0%", "100%"]});
