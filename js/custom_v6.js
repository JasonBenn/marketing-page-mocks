jQuery(document).ready(function(){		
		...
		afterLoad: function(anchorLink, index){
			...
			if(index == 3){
				...
				function swipeCard(){
					...
					//Setup elements
					jQuery('.use-card').css({'background':'url('+backgrounds[counter]+') 50% 50% no-repeat'});
					jQuery('.card-swipe').css({'top':-win_height}); //Move card off screen
					...
					jQuery('#alert').fadeIn(300,function(){
						...
						//Animate card motion
						jQuery('.card-swipe').delay(900).animate({'top':win_height}, 1000,function(){
							jQuery('.card-swipe').css({'top':-win_height});
						});
					});
					...
					//Repeat function once the video has ended
	...
	*/
		...
		function setHeight(){	
			...
			jQuery('.card').width(iphone_width*0.9).css({'margin-left':iphone_width*0.05,'margin-bottom':iphone_width*0.05});			
